# CompileFlow TBBPM Spring Boot Starter

Spring Boot starter for applications using `ProcessEngine` with TBBPM. It combines
`compileflow-spring-boot-starter` and `compileflow-tbbpm`. Durable and Deploy require separate dependencies and
configuration.
...
[TBBPM specification](../docs/en/specifications/tbbpm.md).

```bash
./mvnw test -pl compileflow-spring-boot-starter-tbbpm -am
```
