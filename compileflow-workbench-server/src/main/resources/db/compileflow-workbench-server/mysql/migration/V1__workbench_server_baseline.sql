-- CompileFlow Workbench Server MySQL baseline schema.
--
-- Deploy tables and migration history are owned independently by the selected Deploy Provider.
...
(
    ...
    CONSTRAINT ck_exec_log_source_digest CHECK (
        source_digest IS NULL OR REGEXP_LIKE(source_digest, '^[0-9a-f]{64}$', 'c')
) ,
