...
import com.alibaba.compileflow.engine.extension.spec.*;
 ...
 * 主要方法说明：
 * <ul>
 *   <li>init()：初始化扩展点和扩展实现，自动扫描注册。</li>
 *   <li>getExtensionPoint(String code)：根据扩展点code获取扩展点定义。</li>
 *   <li>getExtensions(String code)：获取指定扩展点下的所有扩展实现。</li>
 *   <li>getExtensions(String code, Class<T> clazz)：获取指定扩展点下指定类型的扩展实现。</li>
 * </ul>
 * <p>
 * 支持SPI、注解、Spring Bean等多种扩展注册方式。
 */
