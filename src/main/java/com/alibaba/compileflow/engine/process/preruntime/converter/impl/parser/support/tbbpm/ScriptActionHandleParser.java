 ...
 * @author yusu
 */
public class ScriptActionHandleParser extends AbstractTbbpmElementParser<ScriptActionHandle> {

    @Override
    protected ScriptActionHandle doParse(XMLSource xmlSource, ParseContext parseContext) throws Exception {
        ScriptActionHandle scriptActionHandle = new ScriptActionHandle();
        String expression = xmlSource.getString("expression");
        if (expression != null) {
            scriptActionHandle.setExpression(expression.trim());
        }
        return scriptActionHandle;
    }

    @Override
    protected void attachChildElement(Element childElement, ScriptActionHandle element, ParseContext parseContext) {

    }

    @Override
    public String getName() {
        return TbbpmModelConstants.SCRIPT_ACTION_HANDLE;
    }
