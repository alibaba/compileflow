...
import com.alibaba.compileflow.engine.bpmn.definition.ScriptTask;
 ...
 * @author yusu
 */
public class ScriptTaskGenerator extends AbstractBpmnActionNodeGenerator<ScriptTask> {

    public ScriptTaskGenerator(GeneratorContext context,
                               ScriptTask flowNode) {
    ...
    public void generateCode(CodeTargetSupport codeTargetSupport) {
        ...
        action.setType(scriptFormat);
        ScriptActionHandle scriptActionHandle = new ScriptActionHandle();
        scriptActionHandle.setExpression(script);
        scriptActionHandle.setVars(flowNode.getVars());
        action.setActionHandle(scriptActionHandle);

        if (shouldExtractMethod(codeTargetSupport)) {
            generateActionMethodCode(codeTargetSupport, action);
        } else {
