...
import com.alibaba.compileflow.engine.bpmn.definition.InclusiveGateway;
...
/**
 * Generates code for a BPMN InclusiveGateway.
 * Evaluates conditional outgoing flows, executes matched branches (possibly in parallel),
 * and falls back to the default flow when no conditions match.
 *
 * @author yusu
 */
public class InclusiveGatewayGenerator extends AbstractGatewayNodeCodeGenerator<InclusiveGateway> {

    public InclusiveGatewayGenerator(GeneratorContext context,
    ...
    protected void generateBranchNodeCode(CodeTargetSupport codeTargetSupport) {
        ...
        SequenceFlow defaultFlow = flowNode.getDefaultTransition();
