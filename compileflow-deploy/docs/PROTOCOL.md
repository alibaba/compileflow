...
(`compileflow-deploy-control-plane`) to the data plane (`compileflow-deploy-runtime`) as two independent streams:
...
## Protocol Generation And Upgrade

The current protocol supports coordinated homogeneous upgrades only. Control-plane writers, channel contents, and
runtime readers must use the same documented generation. Alias-state and artifact parsers accept exactly schema `1`;
both reject unknown fields. CompileFlow does not currently claim an `N/N-1`
mixed-version rolling-upgrade window.

A future skew claim requires concrete adjacent-version readers or writers, immutable compatibility fixtures, rollout
tests in both directions, and tested rollback behavior. A generic codec registry is not part of the protocol until two
real valid representations need to coexist.

In `CHANNEL` mode, publication stores the immutable database fact before projecting it. A failed projection makes the
publish command fail but does not discard or rewrite that fact; an exact retry is idempotent. Before create or rollback
can introduce the version into an Alias, the control plane reloads the persisted winner and confirms the projection
again. A projection failure therefore cannot commit a route that newly references an unavailable artifact.

## 1. Transport And Keys

Distributed topology uses the minimal `DeploymentSyncChannel` contract:
...
`InMemoryDeploymentSyncChannel` is a deterministic test implementation. CompileFlow does not bundle a production remote
...
4. Integration tests exercise competing creates, competing updates, delayed writers, reconnects, and backend failure.

Nacos Config is not listed as a conforming implementation. Its public publish APIs use create-or-update semantics, and a
null CAS digest is not an atomic create-if-absent condition. Its normal client read may also use local failover or
snapshot content. Wrapping those operations in a client-side read/write sequence would weaken this protocol rather than
implement it.
...
coordinator without a remote channel. A remote adapter may impose a stricter key or payload limit, but must validate it
...
The stable share is the residual `10000 - candidateWeightBps`. A stable-only route omits both candidate fields. Unknown
fields are rejected.
...
- A blank channel value means that no route state is present.
...
| `artifactDigest` | string                | yes      | Lowercase SHA-256 over model, code, exact content, and sorted exact direct call bindings  |
...
The first published Version establishes the `modelType` for its `(namespace, code)` process identity. Every later Version
for that identity must use the same model type; a format migration creates a new process code. The field remains on every
Version and artifact so each payload is self-describing at the Data Plane boundary.
...
Release metadata remains in the Control Plane publication record and is deliberately absent from this executable
Data Plane payload. The publish command's optional `expectedArtifactDigest` field owns compare-before-publish integrity
semantics.
...
Route, rollout, audit event, and routing-outbox rows commit in one JDBC transaction. Alias delivery is at least once:

1. The dispatcher publishes only committed outbox rows.
2. Distributed delivery advances channel state with exact-content CAS; a late lower revision is a successful no-op.
3. Consumers independently discard duplicate and lower route revisions because notification order is not authoritative.
4. Reconciliation compares the complete repository route with the channel projection.
5. A missing or lower-revision projection can be replayed with the original route revision, actor, and timestamp.
6. Exact delivery work is coalesced by a database-unique SHA-256 key. Repeated or concurrent reconciliation leaves one
...
8. In `CHANNEL` artifact mode, the same reconciliation cycle deduplicates the stable/candidate versions of all
