...
class EmbeddedAliasRouteConvergerTest {
    ...
    @Test
    void preservesTargetingWhenConvergingAnAuthoritativeCanary() {
        ProcessAliasStore repository = mock(ProcessAliasStore.class);
        AliasTargeting targeting = new AliasTargeting("tenant-policy", Map.of("tenant", "tenant-a"));
        ProcessAliasRecord record = ProcessAliasRecord
            ...
            .updatedAt(2L)
            .build();
        when(repository.resolve(NAMESPACE, CODE, ALIAS)).thenReturn(Optional.of(record));
        LocalRoutingReconciler applier = mock(LocalRoutingReconciler.class);
        when(applier.apply(any())).thenReturn(CompletableFuture.completedFuture(null));
        Duration timeout = Duration.ofSeconds(2);

        try (LocalRoutingConvergence convergence = new LocalRoutingConvergence(applier, 1)) {
            new EmbeddedAliasRouteConverger(repository, convergence, timeout).converge(ProcessRef.alias(NAMESPACE, CODE,
                    ALIAS));
        }

        org.mockito.ArgumentCaptor<DesiredRoutingState> state =
                org.mockito.ArgumentCaptor.forClass(DesiredRoutingState.class);
        verify(applier).apply(state.capture());
        assertThat(state.getValue().getTargeting()).isEqualTo(targeting);
        assertThat(state.getValue().getCandidateWeightBps()).isEqualTo(2_500);
    }
    ...
    void repositoryMissWaitsForTheSharedLocalReadyPipeline() throws Exception {
        ...
        ExecutorService versionRuntimeExecutor = Executors.newSingleThreadExecutor();
        ExecutorService callerExecutor = Executors.newSingleThreadExecutor();
        VersionRuntimeManager manager = VersionRuntimeManager.withOwnedRetryScheduler(resolver, loader,
                versionRuntimeExecutor, localRoutingState, Duration.ofMinutes(1), 1_000);
        LocalRoutingReconciler applier = new LocalRoutingReconciler(manager, localRoutingState, java.util.Set.of());
        try (LocalRoutingConvergence convergence = new LocalRoutingConvergence(applier, 1)) {
            EmbeddedAliasRouteConverger handler =
                    new EmbeddedAliasRouteConverger(repository, convergence, Duration.ofSeconds(2));
            ...
            versionRuntimeExecutor.shutdownNow();
