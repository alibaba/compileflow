...
class DeploymentConfigurationPropertiesTest {
    ...
    void shouldBindTypedDeploymentConfiguration() {
        contextRunner
            .withPropertyValues("compileflow.deploy.database.provider=MYSQL",
                    ...
                    "compileflow.deploy.runtime.convergence-timeout=12s",
                    "compileflow.deploy.runtime.installation-concurrency=2",
            ...
            .run(context -> {
                ...
                assertThat(deployment.getDatabase().getProvider()).isEqualTo(
                        CompileFlowDeploymentProperties.Database.Provider.MYSQL);

                DeploymentArtifactProperties artifact = deployment.getArtifact();
                assertThat(artifact.getMode()).isEqualTo(DeploymentArtifactProperties.Mode.PROJECTION_STORE);
                assertThat(artifact.getOperationTimeout()).isEqualTo(Duration.ofMillis(250));

                assertThat(deployment.getRuntime().getInstallationConcurrency()).isEqualTo(2);
    ...
    @Test
    void shouldLeaveProviderUnsetAndRejectUnknownProvider() {
        contextRunner.run(context -> {
            assertThat(context).hasNotFailed();
            assertThat(context.getBean(CompileFlowDeploymentProperties.class).getDatabase().getProvider()).isNull();
        });
        contextRunner
            .withPropertyValues("compileflow.deploy.database.provider=ORACLE")
    ...
    @Test
    void shouldRejectRemovedProjectionStoreReadTimeoutProperties() {
        contextRunner
    ...
    void shouldRejectRemovedDuplicateRuntimeCapacityConfiguration() {
        contextRunner
            ...
            .run(context -> assertThat(context).hasFailed());
        contextRunner
            .withPropertyValues("compileflow.deploy.runtime.installation-queue-capacity=1")
            .run(context -> assertThat(context).hasFailed());
    }
    ...
    void shouldRejectUnsafeDeploymentWorkBounds() {
        contextRunner
            .withPropertyValues("compileflow.deploy.runtime.installation-concurrency=257")
            .run(context -> assertThat(context).hasFailed());
        contextRunner
    ...
    @Test
    void shouldRejectDistributedOutboxLeaseThatCannotFenceProjectionStoreDelivery() {
        contextRunner
    ...
    @Test
    void shouldRejectProjectionStoreArtifactModeInEmbeddedTopology() {
        contextRunner
            .withPropertyValues("compileflow.deploy.enabled=true", "compileflow.deploy.topology=EMBEDDED",
                    "compileflow.deploy.artifact.mode=PROJECTION_STORE")
            .run(context -> assertThat(context).hasFailed());
