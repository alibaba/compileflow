 ...
 * @author yusu
 */
public final class DeployCursorCodec {
     ...
     */
    public static PublishedVersionKey publishedVersion(PublishedVersionCursor cursor) {
        if (cursor == null) {
            return null;
        }
        String[] fields = decode(cursor.value(), VERSION);
        return new PublishedVersionKey(parseTime(fields[1]), ProcessIdentifiers.requireVersion(fields[2]));
    }
     ...
     */
    public static RolloutKey rollout(RolloutCursor cursor) {
        if (cursor == null) {
            return null;
        }
        String[] fields = decode(cursor.value(), ROLLOUT);
        return new RolloutKey(parseTime(fields[1]), RolloutConstraints.requireId(fields[2], "rolloutId"));
    }
    ...
    /**
     * Decoded keyset boundary for published versions.
     */
    public record PublishedVersionKey(long createdAt, String version) {}

    /**
     * Decoded keyset boundary for rollout history.
     */
    public record RolloutKey(long createdAt, String rolloutId) {}
}
