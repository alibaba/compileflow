...
public final class RoutingOutboxDispatcher {
    ...
    public RoutingOutboxDispatcher(RoutingOutboxStore outboxRepository, RoutingStateDeliveryTarget deliveryTarget,
    ...
    private int dispatchClaimed(RoutingOutboxRecord record) {
        ...
        try {
            ...
            // The projection store accepted the payload, but the row remains PROCESSING until its
            // lease expires. A later claim delivers it again, preserving at-least-once.
            LOGGER.warn("Delivered to projection store but failed to mark as DELIVERED (will retry): "
