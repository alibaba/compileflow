 ...
 *
 * <p>A propagation failure is visible to the command caller, but cannot roll back
 * the committed control-plane state.</p>
 *
 * @author yusu
 */
@FunctionalInterface
public interface RoutingStatePropagator {
    /**
     * Propagates the latest committed state for the affected route.
     *
     * @param alias authoritative alias state read after commit
     */
    void propagate(ProcessAliasState alias);
}
