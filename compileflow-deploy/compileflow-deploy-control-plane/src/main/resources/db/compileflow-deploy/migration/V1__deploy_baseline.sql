-- CompileFlow Deploy schema for PostgreSQL.
--
-- This migration is the sole production DDL owner for the deploy subsystem.
-- Applications embedding the deployment control plane must include this Flyway location.

CREATE FUNCTION cf_reject_immutable_fact_mutation()
    RETURNS TRIGGER
    LANGUAGE plpgsql
AS
$$
BEGIN
    RAISE EXCEPTION USING
        ERRCODE = '55000',
        MESSAGE = format('%I is immutable: %s is not allowed', TG_TABLE_NAME, TG_OP);
END;
$$;
...
(
    ...
    targeting_parameters TEXT         NOT NULL DEFAULT '{}',
...
CREATE INDEX idx_process_alias_process
...
CREATE INDEX idx_rollout_process_created
    ON cf_rollout (namespace, code, created_at DESC, id DESC);
CREATE INDEX idx_rollout_alias_created
    ON cf_rollout (namespace, code, alias, created_at DESC, id DESC);
CREATE UNIQUE INDEX uq_rollout_active_alias
...
CREATE INDEX idx_rollout_event_created
    ON cf_rollout_event (rollout_id, created_at);

CREATE TRIGGER trg_rollout_event_append_only
    BEFORE UPDATE OR DELETE
    ON cf_rollout_event
    FOR EACH ROW
EXECUTE FUNCTION cf_reject_immutable_fact_mutation();
...
CREATE UNIQUE INDEX uq_outbox_delivery_key
