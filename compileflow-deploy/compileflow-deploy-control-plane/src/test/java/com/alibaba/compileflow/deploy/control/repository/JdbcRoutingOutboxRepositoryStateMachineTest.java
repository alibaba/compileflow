...
class JdbcRoutingOutboxRepositoryStateMachineTest {
    private DataSource dataSource;
    private JdbcRoutingOutboxRepository repository;

    @BeforeEach
    void setUp() {
        dataSource = H2TestDatabase.createInMemoryDataSource();
        repository = new JdbcRoutingOutboxRepository(dataSource);
    }
    ...
    void failedDeliveryPersistsBackoffAndReleasesOwnership() throws SQLException {
        ...
        assertThat(repository.markFailed(id, claim.getLeaseToken(), "channel unavailable", 10, 60000L)).isOne();
    ...
    void concurrentEnsuresProduceOneDeliveryRecord() throws Exception {
        ...
        try {
            ...
            Future<Boolean> second = executor.submit(() -> {
                ready.countDown();
                start.await();
                return secondRepository.ensurePendingStandalone(RoutingOutboxRecord.ALIAS_STATE_EVENT_TYPE, "default",
    ...
    private long insertRecord(int revision) {
        return repository.insertPendingStandalone(RoutingOutboxRecord.ALIAS_STATE_EVENT_TYPE, "default", "order.flow",
                "prod", "routing.key", "{\"revision\":" + revision + "}");
    }

    private boolean ensureRecord() {
        return repository.ensurePendingStandalone(RoutingOutboxRecord.ALIAS_STATE_EVENT_TYPE, "default", "order.flow",
                "prod", "routing.key", "{\"revision\":1}");
    }
