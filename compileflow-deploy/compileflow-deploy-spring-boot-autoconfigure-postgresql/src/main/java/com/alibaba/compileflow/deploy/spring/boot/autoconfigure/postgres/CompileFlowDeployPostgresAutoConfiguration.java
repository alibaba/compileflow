...
/**
 * PostgreSQL composition for the complete Deploy persistence authority.
 */
...
@ConditionalOnClass({PostgresDeployStore.class, org.flywaydb.core.Flyway.class})
...
@Conditional(PostgresDeployProviderCondition.class)
public class CompileFlowDeployPostgresAutoConfiguration {
    ...
    public DeployStore deployStore(@Qualifier(DEPLOY_DATA_SOURCE_BEAN_NAME) ObjectProvider<DataSource> deployDataSources,
        ...
        new DeployPostgresSchemaInitializer(dataSource).initialize(properties.getDatabase().isMigrate());
        return new PostgresDeployStore(dataSource, properties.getRouting().getKeyPrefix());
    }
    ...
    static DataSource selectDataSource(ObjectProvider<DataSource> dedicated, ObjectProvider<DataSource> application) {
        ...
        if (dataSource == null) {
            throw new IllegalStateException("CompileFlow Deploy PostgreSQL Provider requires one DataSource");
        }
        return dataSource;
    }
}
