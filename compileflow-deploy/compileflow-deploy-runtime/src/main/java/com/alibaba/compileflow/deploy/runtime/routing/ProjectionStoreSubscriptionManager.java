...
/**
 * Owns a deduplicated set of deployment-projection-store subscriptions.
 *
 ...
 * @author yusu
 */
final class ProjectionStoreSubscriptionManager implements AutoCloseable {
    private static final Logger LOGGER = LoggerFactory.getLogger(ProjectionStoreSubscriptionManager.class);
    private final DeploymentProjectionStore projectionStore;
    private final Duration operationTimeout;
    private final Object lifecycleMonitor = new Object();
    private final Map<String, DeploymentProjectionStore.Subscription> subscriptions = new HashMap<>();
    private boolean closed;

    /**
     * Creates a subscription owner for one projection store.
     *
     * @param projectionStore  projection store on which subscriptions are established
     * @param operationTimeout deadline for establishing one subscription
     */
    ProjectionStoreSubscriptionManager(DeploymentProjectionStore projectionStore, Duration operationTimeout) {
    ...
    private static void closeSubscriptions(List<DeploymentProjectionStore.Subscription> subscriptions) {
        for (DeploymentProjectionStore.Subscription subscription : subscriptions) {
            try {
                subscription.close();
     ...
     * @throws IllegalStateException if this manager is closed or projection store subscription fails
     */
    boolean ensureSubscribed(String key, DeploymentProjectionStore.UpdateCallback callback) {
        ...
        synchronized (lifecycleMonitor) {
            ...
            try {
                DeploymentProjectionStore.Subscription subscription = Objects.requireNonNull(projectionStore.subscribe(subscriptionKey,
                                callback, operationTimeout), "projection store returned a null subscription");
    ...
    private List<DeploymentProjectionStore.Subscription> drainSubscriptions(boolean terminate) {
        synchronized (lifecycleMonitor) {
            if (terminate && closed) {
                return List.of();
            }
            ...
            List<DeploymentProjectionStore.Subscription> owned = new ArrayList<>(subscriptions.values());
            subscriptions.clear();
            return owned;
        }
    }
