...
public final class AliasVersionDemand {
    ...
    public static AliasVersionDemand from(DesiredRoutingState state) {
        DesiredRoutingState desired = Objects.requireNonNull(state, "state");
        if (desired.isDeleted()) {
            return forAlias(desired.getNamespace(), desired.getCode(), desired.getAlias(), Collections.emptySet());
        }
        Set<ProcessRef.Version> versions = new LinkedHashSet<>();
        versions.add(ProcessRef.version(desired.getNamespace(), desired.getCode(), desired.getStableVersion()));
        if (desired.getCandidateVersion() != null) {
            versions.add(ProcessRef.version(desired.getNamespace(), desired.getCode(), desired.getCandidateVersion()));
        }
        return forAlias(desired.getNamespace(), desired.getCode(), desired.getAlias(), versions);
    }
