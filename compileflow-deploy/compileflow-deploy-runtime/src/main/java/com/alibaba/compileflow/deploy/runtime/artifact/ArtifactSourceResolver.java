 ...
 * @author yusu
 */
public final class ArtifactSourceResolver implements ProcessArtifactResolver {
    private final ProcessArtifactSource source;

    public ArtifactSourceResolver(ProcessArtifactSource source) {
    ...
    public ProcessArtifact resolve(ProcessRef.Version key) {
        ...
        try {
            ...
            throw DeploymentException
                .builder(DeploymentErrorCode.STORAGE_ERROR, "Failed to read process artifact from repository", failure)
                .namespace(key.namespace())
                .code(key.code())
                .version(key.version())
                .build();
