 ...
 * @author yusu
 */
public final class ProcessDeploymentMetrics {
     ...
     */
    public ProcessDeploymentMetrics() {
    }
    ...
    /**
     * Returns an immutable point-in-time copy of all observed error counts.
     *
     * @return immutable map ordered by the map's current traversal order
     */
    public Map<DeploymentErrorCode, Long> getErrorsByCode() {
        if (errorByCode.isEmpty()) {
            return Collections.emptyMap();
        }
        Map<DeploymentErrorCode, Long> snapshot = new LinkedHashMap<>();
        errorByCode.forEach((code, counter) -> snapshot.put(code, counter.sum()));
