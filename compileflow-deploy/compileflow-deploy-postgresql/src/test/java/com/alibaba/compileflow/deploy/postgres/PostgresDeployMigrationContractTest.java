...
class PostgresDeployMigrationContractTest {
    private static final String MIGRATION = "/db/compileflow-deploy/postgres/migration/V1__deploy_baseline.sql";

    @Test
    void authoritySchemaPhysicallyProtectsImmutableFacts() throws IOException {
        String sql;
        try (InputStream input = PostgresDeployMigrationContractTest.class.getResourceAsStream(MIGRATION)) {
        ...
        assertThat(sql)
            .doesNotContainPattern("(?i)(call_bindings|targeting_parameters)\\\\s+TEXT\\\\s+NOT " + "NULL\\\\s+DEFAULT");
    }

    @Test
    void baselineBindsModelTypeToEachImmutableVersion() throws IOException {
        String sql;
        try (InputStream input = PostgresDeployMigrationContractTest.class.getResourceAsStream(MIGRATION)) {
            assertThat(input).as("deploy migration").isNotNull();
            sql = new String(input.readAllBytes(), StandardCharsets.UTF_8);
        ...
        assertThat(sql)
            ...
            .doesNotContain("trg_process_identity_immutable");
    }
}
