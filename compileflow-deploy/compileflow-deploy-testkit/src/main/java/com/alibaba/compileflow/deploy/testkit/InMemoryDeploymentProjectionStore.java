...
/**
 * Thread-safe, process-local deployment projection store for deterministic tests.
 *
 ...
 * @author yusu
 */
public final class InMemoryDeploymentProjectionStore implements DeploymentProjectionStore {
    private static final Logger LOGGER = LoggerFactory.getLogger(InMemoryDeploymentProjectionStore.class);
    private final Map<String, String> store = new ConcurrentHashMap<>();
    private final Map<String, CopyOnWriteArrayList<Subscriber>> callbacks = new ConcurrentHashMap<>();

    /**
     * Creates an empty process-local projection store with independent state and subscriptions.
     */
    public InMemoryDeploymentProjectionStore() {
    }
