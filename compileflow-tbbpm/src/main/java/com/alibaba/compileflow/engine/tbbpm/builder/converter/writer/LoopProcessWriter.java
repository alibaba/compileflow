 ...
 * @author yusu
 */
public class LoopProcessWriter extends AbstractTbbpmNodeWriter<LoopProcessNode> {
    ...
    protected void enrichNodeAttr(LoopProcessNode node, XMLStreamWriter xsw) throws Exception {
        if (node.getId() != null) {
            xsw.writeAttribute(TbbpmModelConstants.ATTRIBUTE_ID, node.getId());
        }
        if (node.getName() != null) {
            xsw.writeAttribute(TbbpmModelConstants.ATTRIBUTE_NAME, node.getName());
        }
    }
