...
import com.alibaba.compileflow.engine.core.definition.Stateless;
...
public class LoopProcessNode extends FlowNode implements NodeContainer<FlowNode>, Stateless {
    ...
    public FlowNode getNode(String id) {
        return flowNodes.stream().filter(node -> id.equals(node.getId())).findFirst()
                .orElseThrow(() -> new CompileFlowException.BusinessException(ErrorCode.CF_RESOURCE_001, "Undefined node, node id is " + id));
    }
    ...
    public FlowNode getStartNode() {
        return flowNodes.stream().filter(node -> node.getId().equals(startNodeId)).findFirst()
                .orElseThrow(() -> new CompileFlowException.BusinessException(ErrorCode.CF_VALIDATION_002, "No start node found"));
    }

    @Override
    public FlowNode getEndNode() {
        return flowNodes.stream().filter(node -> node.getId().equals(endNodeId)).findFirst()
                .orElseThrow(() -> new CompileFlowException.BusinessException(ErrorCode.CF_VALIDATION_002, "No end node found"));
    }
