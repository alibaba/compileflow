...
import com.alibaba.compileflow.engine.tbbpm.definition.WaitTaskNode;

/**
 * @author wuxiang
 * @author yusu
 */
public class WaitTaskParser extends AbstractTbbpmElementParser<WaitTaskNode> {

    @Override
    protected WaitTaskNode doParse(XMLSource xmlSource, ParseContext parseContext) throws Exception {
        WaitTaskNode waitTaskNode = new WaitTaskNode();
        waitTaskNode.setId(xmlSource.getString(TbbpmModelConstants.ATTRIBUTE_ID));
        waitTaskNode.setName(xmlSource.getString(TbbpmModelConstants.ATTRIBUTE_NAME));
        waitTaskNode.setTag(xmlSource.getString(TbbpmModelConstants.ATTRIBUTE_TAG));
        waitTaskNode.setDescription(xmlSource.getString(TbbpmModelConstants.ATTRIBUTE_DESCRIPTION));
        waitTaskNode.setG(xmlSource.getString(TbbpmModelConstants.ATTRIBUTE_G));
        return waitTaskNode;
    }
