...
export function useAsyncInvocationLedger(refreshToken: number): AsyncInvocationLedgerState {
  ...
  const query = useRef({ page, status, processCode })
  query.current = { page, status, processCode }
  ...
  const loadInvocations = useCallback(async (background = false): Promise<boolean> => {
    if (!mounted.current) return true
    const { page, status, processCode } = query.current
  ...
  }, [loadInvocations, page, processCode, refreshToken, status])
  ...
  const replaceInvocation = useCallback((accepted: AsyncInvocationResponse) => {
    if (!mounted.current) return
    setInvocations((current) => {
      if (!current.data.some((item) => item.invocationId === accepted.invocationId)) {
        return current
      }
      const { status, processCode } = query.current
      const filteredOut =
        (status !== undefined && accepted.status !== status) ||
        (processCode !== undefined && accepted.processCode !== processCode)
      return {
        data: filteredOut
