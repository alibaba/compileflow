...
@Service
public class PaymentFundApportionActivity extends BaseOmActivity {
    @Override
    protected void doExecute(BusinessContext businessContext) {
        // Apportion payment funds
        // Double amount = businessContext.getOrderAmount();
        Double amount = businessContext.getOrderAmount();
        if (amount != null && amount > 0) {
            businessContext.put("fundApportioned", true);
            businessContext.put("apportionAmount", amount);
        }
    }
}
