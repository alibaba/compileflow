...
public class SecurityManualCheckCallbackActivity extends BaseOmActivity {
    ...
    protected void doExecute(BusinessContext businessContext) {
        // Handle security manual check callback
        // String orderId = businessContext.getOrderId();
        String orderId = businessContext.getOrderId();
        if (orderId != null) {
            businessContext.put("securityManualCheckStatus", "completed");
            businessContext.put("securityCheckTime", System.currentTimeMillis());
        }
    }
}
