...
@Service
public class OrderReverseActivity extends BaseOmActivity {
    @Override
    protected void doExecute(BusinessContext businessContext) {
        // Reverse order (cancellation or return)
        // String orderId = businessContext.getOrderId();
        String orderId = businessContext.getOrderId();
        if (orderId != null) {
            businessContext.put("reverseStatus", "processing");
            businessContext.put("reverseReason", "cancelled");
        }
    }
}
