...
@Service
public class OrderGroupCreateActivity extends BaseOmActivity {
    @Override
    protected void doExecute(BusinessContext businessContext) {
        // Create order group for batch processing
        // String orderId = businessContext.getOrderId();
        String orderId = businessContext.getOrderId();
        if (orderId != null) {
            businessContext.put("orderGroupId", "GROUP-" + orderId);
            businessContext.put("groupCreated", true);
        }
    }
}
