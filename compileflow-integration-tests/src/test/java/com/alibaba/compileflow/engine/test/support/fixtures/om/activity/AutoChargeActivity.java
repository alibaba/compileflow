...
@Service
public class AutoChargeActivity extends BaseOmActivity {
    @Override
    protected void doExecute(BusinessContext businessContext) {
        // Auto charge payment for subscription orders
        // Double amount = businessContext.getOrderAmount();
        Double amount = businessContext.getOrderAmount();
        if (amount != null && amount > 0) {
            businessContext.put("chargeStatus", "processing");
            businessContext.put("chargeAmount", amount);
            businessContext.put("chargeMethod", "auto");
        }
    }
}
