...
/**
 * Persistent iteration position for one active while loop.
 *
 * @author yusu
 */
public record WhileFrame(String loopId, int position) implements ScopeFrame {
    public WhileFrame {
        loopId = requireText(loopId);
        if (position < 0) {
            throw new IllegalArgumentException("while position must be non-negative");
        }
    }
