...
/**
 * Supplies opt-in generated-source and bytecode export for Durable tests.
 *
 * <p>Set {@value #DEBUG_ENABLED_PROPERTY} to {@code true} for a local test
 * run. Artifacts are written below {@code target/compileflow-debug} unless
 * {@value #DEBUG_OUTPUT_DIRECTORY_PROPERTY} supplies another directory.</p>
 *
 * @author yusu
 */
public final class DurableTestCompilation {
    public static final String DEBUG_ENABLED_PROPERTY = "compileflow.test.compilation.debug";
    public static final String DEBUG_OUTPUT_DIRECTORY_PROPERTY = "compileflow.test.compilation.debug-output-directory";
    private static final Path DEFAULT_DEBUG_OUTPUT_DIRECTORY = Path.of("target", "compileflow-debug");

    private DurableTestCompilation() {
    }

    /**
     * Returns the compilation configuration for the current test JVM.
     *
     * @return in-memory defaults, or full debug export when explicitly enabled
     */
    public static JavaDiagnosticsConfig config() {
