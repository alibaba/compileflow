# CompileFlow 2.0 Durable 术语

以下术语适用于 Durable 公共 API 与架构文档。通用引擎和部署词汇见[项目术语表](../zh/glossary.md)。Java
...
| 术语 | 含义 |
|---|---|
| 流程定义（Process definition） | 调用方提供的模型内容。`ProcessDefinition.Inline` 包含流程 code 和文本，不携带版本、Alias、模型类型或部署元数据。 |
| 流程制品（Process artifact） | Deploy 管理的不可变发布内容，包括精确版本、校验后的定义和发布元数据。Durable 不再公开另一套制品包装类型。 |
