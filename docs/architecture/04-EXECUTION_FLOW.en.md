...
Sections 1–14 describe the `ProcessEngine`. Durable `Start/Trigger` does not reuse this invocation
lifecycle; its persisted execution flow is described separately in section 15.
...
The definition code and version-reference code must match. Direct definitions do not acquire an implicit namespace,
version, Alias, or model type beyond the documented default namespace and the receiving format-bound Engine.
...
`ProcessError`. Start events contain only process code and invocation ID. Events cannot carry routing keys, process
...
- multi-format dispatch retains the selected root runtime before entering the format-bound engine;
...
The format-specific `ProcessRuntimeFactory` performs:
...
session directory, and must not alter Runtime identity or execution behavior.
...
Direct `execute`, multi-format Alias dispatch, nested calls, and `trigger` use this same boundary. The multi-format
Registry holds one lock-free admission token across route selection, format
handoff, and execution. Shutdown atomically stops admission and scans stable non-negative counter stripes, so it waits
for admitted calls to finish. The same remaining deadline is passed to each Engine rather than restarting the grace
period at every layer.
...
`ProcessToolingService.preflight(...)` performs non-executing validation and optional dry-run compilation. Parallel
warm-up compilation is used only when configured and useful for multiple definitions. Admin load and warm-up are not
...
start(explicit Definition | exact Version | Alias)
  ...
  -> current deployment loads disposable code and current capabilities
  ...
  -> Trigger / Timer / Effect result creates one resume envelope
...
the [Durable Architecture](10-DURABLE_ARCHITECTURE.en.md) for full sequences, state machines, failure recovery, and data
ownership.

## Next Steps

- [Version Routing](05-VERSION_ROUTING.en.md)
- [Durable Architecture](10-DURABLE_ARCHITECTURE.en.md)
- [Advanced Features](../en/advanced-features.md)
- [Resource Management](../en/resource-management.md)
