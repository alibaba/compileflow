...
CompileFlow is a format-bound, compile-then-execute process engine. Optional products add hot deployment, persisted
...
flowchart TB
    ...
    DeployAPI["compileflow-deploy-api<br/>commands, domain contracts, wire codecs"]
...
programs, bytecode, derived schemas, and current
application providers are disposable runtime material. Each bounded Machine Turn commits the complete next
continuation, exact occurrence consumption, new occurrence requests, and disposition atomically to PostgreSQL.
...
See the [Durable Architecture](10-DURABLE_ARCHITECTURE.en.md) for the complete component, state, transaction, security,
...
the [Extension Guide](../en/extension-guide.md).

## Configuration

External configuration is parsed once at its boundary:

| Prefix                           | Owner                         |
|----------------------------------|-------------------------------|
| `compileflow.engine.*`           | Engine configuration          |
| `compileflow.deploy.*`           | Deployment control/data plane |
| `compileflow.workbench.server.*` | Server product settings       |
| `COMPILEFLOW_DEV_GATEWAY_*`      | Loopback development mock     |
| `VITE_COMPILEFLOW_*`             | Public web build-time inputs  |

Spring's own datasource, server, management, and logging settings remain under their standard prefixes. Unknown
CompileFlow Spring properties fail binding.
