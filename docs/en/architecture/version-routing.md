...
> Prerequisites: [Architecture overview](overview.md) and
...
digest, metadata, exact call bindings, actor, and database creation time. Model type belongs to each exact Version;
different Versions of the same process code may use different frontends. Row existence is the publication fact;
there is no mutable preparation status. Exact Version ProcessCalls may cross frontend boundaries and must satisfy
the shared semantic input/output contract. Classpath calls inherit the caller's frontend.
...
projection store. The default `LocalReadyAliasRouteSource` exposes the alias projection as serving-ready state; a database
...
The context does not expose candidate weight, and the engine maps a target to an immutable authorized version.
...
converged. Delivery is at least once, and reconciliation republishes current authority when the projection store drifts.
...
- Inspect `DeploymentRuntime.snapshot()` for demanded, in-flight, deployed, and backed-off versions.
