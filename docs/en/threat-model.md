...
| Build and distribution                 | Authorized source, pinned dependencies, and protected build credentials                           | JARs, source archives, SBOMs, checksums, and signed provenance                                                                                                                   |
...
execution variables, Durable snapshots, Effect payloads, Wait tokens, and Outbox records; database, API, signing, and
build credentials; audit records; and distributed artifacts with their SBOM and provenance identity.

Relevant actors are:

- maintainers and build operators with repository or signing authority;
...
5. Protected CI builds authorized source with pinned tools and dependency manifests. Distributed artifacts are bound
   to checksums and provenance.
...
| TM-05 | An artifact is substituted, stale routing is accepted, or rollback targets the wrong code                          | Immutable Versions carry content identity; publication and routing use explicit state transitions, optimistic checks, and exact targets; distributed artifacts have checksums and provenance                                            | Restrict publication authority, secure artifact transport and storage, inspect rollouts, and monitor reconciliation failures                                              |
...
| TM-10 | A dependency, CI action, base image, wrapper, or distributed artifact is replaced                                  | Lockfiles and managed manifests, checksummed Maven Wrapper downloads, commit-pinned Actions, digest-pinned images, dependency scanning, CodeQL, SpotBugs, SBOMs, checksums, and artifact attestations protect the repository-owned path | Protect repository settings and CI secrets with MFA; verify artifact provenance; assess registry, runner, and build-service trust                                         |
...
The project publishes separate dependency inventories for Maven libraries, the Workbench backend, and the production
browser bundle. The server inventory covers both the server and all-in-one JARs, including embedded libraries and the
Spring Boot loader. Checksums and provenance bind each inventory to its distributed artifacts.

Security findings must not be silently suppressed. Record a non-exploitable finding in a CycloneDX VEX document tied
to the affected SBOM. Report suspected vulnerabilities through the private channel documented in the
