...
the threats the project mitigates, the controls owned by an embedding application or deployment,
and the residual risks that a release review must not hide. Support level and release status remain defined by
[Supported Surfaces](../architecture/06-SUPPORTED_SURFACES.en.md); the [Security Guide](security.md) provides the
procedural configuration guidance.
...
- unauthenticated network clients, malicious or compromised dependencies, and attackers controlling an unreviewed
  definition, browser request, database row, artifact channel, or CI metadata.
...
2. An authenticated deployment command stores immutable artifact identity and changes routing state; a runtime resolves
...
5. CI builds a reviewed commit with pinned tools and dependency manifests. A tag release assembles all assets as a
   draft, binds their hashes to provenance, and publishes only the complete set.
...
| TM-10 | A dependency, CI action, base image, wrapper, or release asset is replaced | Lockfiles and managed manifests, checksummed Maven Wrapper downloads, commit-pinned Actions, digest-pinned images, dependency review, CodeQL, SpotBugs, SBOMs, checksums, and SLSA provenance protect the repository-owned path | Protect repository settings and CI secrets with MFA and review; verify release provenance; assess registry, runner, and build-service trust |
...
Review this model for every major security-boundary change and before each stable minor or major release. Changes to
