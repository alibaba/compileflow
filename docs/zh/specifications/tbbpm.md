...
对应 Schema 位于 `compileflow-tbbpm/src/main/resources/TBBPM.xsd`。支持 Java 17、21 和 25。
...
组件解析器和脚本 Provider 可能被不同分支或不同流程调用并发使用，其生命周期管理方必须保证实例隔离或线程安全。
分支失败无法回滚外部副作用，因此外部动作仍必须使用匹配的 Effect 协议。

进入并发区域时，`inclusive` 会先在父流程状态上按模型顺序计算全部条件，再启动符合条件的分支。
...
变量；一个分支读取而另一个分支显式写入时，读取方看到分支创建时的快照，不构成冲突。可变 `List`、`Map`、数组、实体或普通 POJO
不得被原地修改。需要聚合时，应让分支写入不同的中间变量，汇合后再串行计算最终值，不依赖完成顺序、共享集合或后写覆盖。
...
每个脚本输入和输出都用 `<input>` 或 `<output>` 显式声明。Java Code 是一段方法体，例如
...
Java Code 是可信的进程内代码，不提供安全沙箱。Workbench Server 不得执行不可信作者提交的代码；应用如需接收此类代码，
应在 CompileFlow 部署之外使用操作系统或容器隔离的执行环境。
