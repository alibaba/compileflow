...
优雅停机时，工作节点先停止领取任务并取消延迟探测，再等待正在执行的 Turn、Effect 和 Outbox 任务完成。这些调用不会被中断，Spring 生命周期排空期间仍会续租。`spring.lifecycle.timeout-per-shutdown-phase` 与容器终止宽限期必须大于最长的在途调用时间。超过期限后进程若被强制终止，将由租约过期和 fencing 机制恢复；对外部系统可见的操作仍须保持幂等。
