...
| 重启后从持久化节点恢复流程        | CompileFlow Durable       | 需要 PostgreSQL、MySQL 或符合契约的自定义存储 |
...
- **非 JVM 运行时。** 执行环境需要 Java 17 或更高版本，并包含 `jdk.compiler` 模块。
...
| 需要托管人工任务或完整 BPMN 标准吗？               | 超出支持范围                      | 继续评估                 |
| 需要不可变版本和灰度路由吗？                       | 增加 CompileFlow Deploy           | 使用内联或类路径定义     |
| 应用能否运行在含 `jdk.compiler` 的 Java 17+ 环境？ | 继续评估                          | 不支持该运行环境         |
