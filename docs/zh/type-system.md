...
范围明确、结果确定的运行时转换。非法或含糊的值会直接失败，不会静默截断、猜测地区设置或回退默认值。
...
`java.lang` 之外的类型应使用全限定名。内建简写只包括 `String`、Java 基本类型和标准 `java.lang`
包装类型。每个泛型参数类型都必须对编译类加载器可见，泛型参数数量必须与目标类声明一致。后缀格式错误、类不存在、
使用基本类型作为泛型参数（数组是合法引用类型）、大小写错误或数组维度超过 JVM 上限，都会使预检失败。成员类在流程定义和
生成源码中使用规范名称（例如 `java.util.Map.Entry`），加载时再转换为 JVM 所需的二进制名称。
...
| `BigDecimal`                                         | `Number` 或十进制文本                | 只接受与地区设置无关的 Java 十进制语法                                                    |
| `boolean`                                            | `Boolean` 或 `true` / `false` 文本   | 不猜测数字、yes/no 或 on/off                                                              |
| `char`                                               | `Character` 或一个 UTF-16 代码单元   | 空文本和多字符文本失败                                                                    |
...
| `java.util.Date`                                     | `Instant` 或 ISO 时间文本            | 必须提供时区或 UTC 偏移量，且只能使用毫秒精度；本地日期时间文本和会丢失的纳秒精度均被拒绝 |

转换不读取宿主的地区设置或默认时区。转换异常只给出来源类型和目标类型，不回显可能包含凭据或个人信息的运行时值。

## 默认值

变量默认值在生成 Java 源码时完成校验：

- 基本类型、包装类型、字符串、`BigInteger`、`BigDecimal`、`java.time` 和 JDBC 时间字面量均严格解析。
- `DataTypes` 直接处理基本类型时，未提供默认值使用 Java 零值；流程变量声明中的基本类型别名会
  规范化为对应包装类型，因此流程状态保持可空语义，未提供默认值为 `null`。
- 输入 Map 缺少对应键时保留声明的默认值；键存在时覆盖默认值。因此显式 `null` 与未传值语义不同。
- 空 `String` 是有效的空字符串默认值；其他类型的空白默认值视为未提供。
- 所有默认值都是数据字面量，`@` 没有特殊语义。例如 String 默认值 `@name` 会原样保留；同一文本用于不兼容的数值或时间
  类型时会校验失败。
- 不支持的对象默认值会使预检失败；应改为通过输入上下文传值。
