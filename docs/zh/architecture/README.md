...
本组文档介绍 CompileFlow 的组件边界、执行模型和支持范围。具体契约以[支持面清单](supported-surfaces.md)、
...
| 公共与内部契约       | [支持面清单](supported-surfaces.md)     |
...
- **了解系统：** 架构概览 → 模块地图 → 执行流程 → 支持面清单。
- **嵌入引擎：** [快速开始](../quick-start.md) → [配置指南](../configuration.md) → 支持面清单。
- **接入 Durable：** [Durable 使用指南](../durable-process.md) → Durable 架构 →
  [值班与恢复手册](../durable-operations-runbook.md)。
- **扩展引擎：** [扩展指南](../extension-guide.md) → 模块地图 → 支持面清单。
- **修改实现：** 模块地图 → [参与贡献（英文）](../../../CONTRIBUTING.md) → [测试指南](../testing.md)。

架构文档用于说明实现，不扩展[支持面清单](supported-surfaces.md)中的兼容性承诺。本文档与项目均采用
