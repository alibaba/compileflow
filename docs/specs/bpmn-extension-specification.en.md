...
in the [Node Support List](../en/node-support.md).
...
All child extensions appear under the BPMN element's `extensionElements`.
...
An Action with `execution="effect"` may contain one `cf:effectPolicy` after its `cf:action`. Omitting the policy
...
The BPMN writer preserves the executable subset and writes CompileFlow data in the `cf:` namespace. Default
...
When changing this contract, update the extension XSD, parser, model validator, writer, generator or executor where
applicable, this bilingual specification, and strict parse plus round-trip tests in the same change.
