...
> **Status**: Canonical for CompileFlow 2.0.
...
Every executable or diagram node has an `id` and a geometry attribute `g`. Node IDs must be globally unique across the
...
`waitEventTask`, the requested event must match. Wait nodes deliberately have no `inAction` or `outAction`; authors use
...
`timerTask` suspends a Durable Run until a database-clock-derived due time:
...
The task and Action remain the business model; no second Effect target identity is introduced. The current deployment
...
[Durable Process guide](../en/durable-process.md) for registration and runtime semantics.
...
caller-relative resolution. On the
`ProcessEngine`, the called process runs synchronously. Nested process variables map caller context to and from the
called process and therefore support only `param` and `return`. To pause a ProcessEngine caller after the call, connect the
`bpmCall` to an explicit `waitTask` or `waitEventTask`; process invocation and external correlation are deliberately
separate semantics.
...
distinct declared process variables. `target` must be `java.util.List`-compatible, `source` must name an
...
Parallel issue is a bounded rolling window controlled by
`compileflow.durable.worker.max-active-iterations` (`1..64`, default `32`). This value is current Runtime scheduling
...
| ------------- | ----------------------------------------------------- | ------------------------------------------------------------- |
| `java`        | `class`, `method`, optional `input`/`output` children | Invoke an application-owned reusable Java capability.         |
| `spring-bean` | `bean`, `class`, `method`, optional mappings           | Invoke an application-owned Spring capability.                |
...
semantics tolerate the selected ProcessEngine policy. Durable execution ignores this invocation policy and uses
...
inputs, and executes the compiled class. Repeated requests may reuse that exact runtime. Triggered execution uses the
...
- Accepts only XML admitted by the current XSD and semantic validator.
...
- Keeps wait entries at the top level.
- Compiles generated Java with release 17.
- Does not claim durable resume, message correlation, or persistent human-task semantics.

## 9. Related Documents

- [Process Model Architecture](../architecture/07-PROCESS_MODEL.en.md)
- [TBBPM vs BPMN](tbbpm-vs-bpmn.md)
- [Java API Reference](../en/api-reference.md)
- [Node Support](../en/node-support.md)
