...
/**
 * Steady-state throughput of exact-version execution on a loaded BPMN 2.0 flow.
 * The flow is the BPMN 2.0 equivalent of the TBBPM
 * {@code hello.bpm} used by {@link CompileFlowExecuteBenchmark}: it computes {@code value + 2}
 * via a QL {@code scriptTask} and returns it.
 *
 * <p>Pair this benchmark with {@link CompileFlowExecuteBenchmark} to verify the
 * steady-state parity of TBBPM and BPMN on the same compile-then-execute engine.
 * Parsing and code generation happen during setup, outside the measured operation;
 * a divergence therefore points to generated runtime behavior or execution shape.
 *
...
@Fork(value = 1)
public class BpmnExecuteBenchmark {
